...
void add_sphere( struct matrix * edges, 
                 ...
                 double r, int step ) {
                   int i;
                   struct matrix* ballpoints = generate_sphere(cx, cy, cz, r, step);
...
struct matrix * generate_sphere(double cx, double cy, double cz,
                                double r, int step ) {
                                  struct matrix* ballpoints = new_matrix(4, 50000000);
                                  int phi, theta;
                                  ...
                                  ballpoints->lastcol = phi * step + theta + 1;
  return ballpoints;
}
...
struct matrix * generate_torus( double cx, double cy, double cz,
                                double r1, double r2, int step ) {
                                  struct matrix* donut = new_matrix(4, 50000000);
                                  int phi, theta;
                                  double phi_t, theta_t;
                                  ...
                                  donut->lastcol = phi * step + theta + 1;
